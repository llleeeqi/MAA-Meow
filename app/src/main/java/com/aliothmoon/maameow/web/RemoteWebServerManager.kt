...
<div class="toolbar"><input id="password" type="password" placeholder="访问密码"><input id="search" placeholder="搜索设置项"><button id="load">读取设置</button><button id="save">保存更改</button><button id="statusButton" class="secondary">运行状态</button></div>
<div id="status" class="status">请输入密码后读取设置。</div><main id="editor" class="root"></main>
<script>
let config=null;const editor=document.querySelector('#editor'),statusBox=document.querySelector('#status'),searchBox=document.querySelector('#search');
...
const choices={overlayMode:['ACCESSIBILITY','SHIZUKU','ROOT'],runMode:['BACKGROUND','FOREGROUND'],updateSource:['GITHUB','MIRROR_CHYAN'],startupBackend:['SHIZUKU','ROOT','ACCESSIBILITY'],updateChannel:['STABLE','BETA'],themeMode:['SYSTEM','LIGHT','DARK'],language:['ZH','EN']};
...
function inputFor(value,key,change){const originalType=typeof value;let input;
 if(bool(value)){input=document.createElement('input');input.type='checkbox';input.className='switch';input.checked=value===true||value==='true';input.onchange=()=>change(originalType==='string'?String(input.checked):input.checked);return input}
 if(choices[key]){input=document.createElement('select');choices[key].forEach(x=>{const o=document.createElement('option');o.value=x;o.textContent=x;input.append(o)});if(!choices[key].includes(String(value))){const o=document.createElement('option');o.value=value;o.textContent=value;input.append(o)}input.value=value;input.onchange=()=>change(input.value);return input}
 input=document.createElement(numeric(value)?'input':'textarea');if(input.tagName==='INPUT')input.type='number';else input.rows=String(value).length>90?4:1;input.type=secret(key)?'password':input.type;input.value=value??'';input.oninput=()=>{let v=input.value;if(originalType==='number')v=Number(v);change(v)};return input}
...
function render(value,key,change,depth=0){if(value===null||typeof value!=='object')return field(key,value,change);if(Array.isArray(value)){const [box,body]=group(label(key)+'（'+value.length+'）',depth<2);const draw=()=>{body.innerHTML='';if(!value.length){const empty=document.createElement('div');empty.className='empty';empty.textContent='暂无项目';body.append(empty)}value.forEach((item,index)=>{const wrap=document.createElement('div');wrap.className='array-item';const actions=document.createElement('div');actions.className='array-actions';const remove=document.createElement('button');remove.className='danger';remove.textContent='删除';remove.onclick=()=>{value.splice(index,1);draw()};actions.append(remove);wrap.append(actions);wrap.append(render(item,'项目 '+(index+1),next=>{value[index]=next},depth+1));body.append(wrap)});const add=document.createElement('button');add.className='secondary';add.textContent='添加项目';add.onclick=()=>{value.push(value.length?JSON.parse(JSON.stringify(value[value.length-1])):'');draw()};body.append(add)};draw();return box}
...
async function load(){setStatus('正在读取…');const r=await fetch('/api/config',{headers:auth()});if(!r.ok){setStatus(await r.text(),true);return}config=await r.json();draw();setStatus('已加载。修改表单后点击“保存更改”。')}
async function save(){if(!config)return setStatus('请先读取设置。',true);if(!confirm('确认写回当前网页中的全部设置？'))return;setStatus('正在保存…');const r=await fetch('/api/config',{method:'PUT',headers:{...auth(),'Content-Type':'application/json'},body:JSON.stringify(config)});const text=await r.text();if(!r.ok){setStatus(text,true);return}setStatus('保存成功。')}
...
document.querySelector('#load').onclick=load;document.querySelector('#save').onclick=save;document.querySelector('#statusButton').onclick=showStatus;searchBox.oninput=applySearch;
</script></body></html>
"""
